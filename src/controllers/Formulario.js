...
function diaDeLaSemana(Inicio, Final) {
    ...
    while (Final == 0) {
        Dia = new Date(Inicio.getFullYear(), Inicio.getMonth() + 1, 0 - i);
...
function GenerarFormulario(req, res) {
    ...
    var fechaHoy = generoPoesia1.toISOString().split('T')[0];
    ...
    Formulario.find({ carnet: parametros.carnet }, (err, formularioEncontrado) => {
        if (formularioEncontrado.length > 0) {
            ...
            return res.status(500).send({ mensaje: "No se puede repetir este formulario" })
                    ...
                    if (carnet.charAt(carnet.length - 1) === "1" || carnet.charAt(carnet.length - 1) === "3" || carnet.charAt(carnet.length - 1) === "9") {
                        if (parametros.carnet && parametros.nombre && parametros.direccion &&
                            ...
                            if (String(calcularEdad(parametros.fechaNacimiento)) > 17) {
                                ...
                                modeloFormulario.fechaInscripcion = fechaHoy;
